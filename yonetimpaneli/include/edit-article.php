...
if (!empty($_GET["ID"])) {
    ...
    $data = $VT->VeriGetir("articles", "WHERE ID=?", array($ID), "");
    if ($data != false) {
?>
        ...
        <div class="container-fluid">
            ...
            if ($_POST) {
                if (!empty($_POST["title"])) {
                    ...
                    $ekle = $VT->SorguCalistir("UPDATE " . "articles", "SET lang_id=?, title=?, seflink=?, description=?, image=?, status=? WHERE ID=?", array($lang_id, $title, $seflink, $description, $image, $status, $ID),1);
                    ...
                    <div class="card-body card card-primary">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" style="width: 100%;" name="lang_id">
                                        ...
                                        ?>
                                                <option <?php if($data[0]["lang_id"]==$language["lang_id"]){echo "selected";} ?> value="<?= $language["lang_id"] ?>"><?= $language["lang_title"] ?></option>
                                        <?php
                                            }
                                        }
                                        ?>
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label>Başlık</label>
                                    <input type="text" class="form-control" placeholder="Başlık ..." name="title" value="<?=stripslashes($data[0]["title"])?>">
                                </div>
                            </div>
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label>Açıklama</label>
                                    <textarea class="textarea" name="description" id="" cols="30" rows="10"><?=stripslashes($data[0]["description"])?></textarea>
                                </div>
                            </div>
                            <div class="col-md-12">
                                <div class="form-group">
