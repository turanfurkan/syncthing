...
{
  $user=$VT->VeriGetir("users","WHERE ID=?",array($_SESSION["ID"]),"");
...
<head>
  ...
  <script src="https://kit.fontawesome.com/f532863a92.js" crossorigin="anonymous"></script>

  <link rel="stylesheet" href="<?= SITE ?>plugins/dropzone/dropzone.css">
...
<script src="<?= SITE ?>plugins/dropzone/dropzone.js"></script>
...
<script>
  ...
  function aktifpasif(lang_id,tablo)
  {
	  var lang_status=0;
	 if($(".aktifpasif"+lang_id).is(':checked'))
	 {
		 lang_status=1;
	 }
	 else
	 ...
	 $.ajax({
		 method:"POST",
		 url:"<?=SITE?>ajax.php",
		 data:{"tablo":tablo,"lang_id":lang_id,"lang_status":lang_status},
